...
const JobDescription = () => {
    ...
    const applyJobHandler= async ()=>{
           try{
            const res=await fetch(`https://jobhunt1-lheo.onrender.com/api/v1/application/apply/${jobId}`,
                {
                    method:"GET",
                    credentials:"include"
                }
    ...
    useEffect(() => {
        const fetchSingleJob = async () => {
            try {
                const res = await fetch(`https://jobhunt1-lheo.onrender.com/api/v1/jobs/get/${jobId}`, {
                    method: 'GET',
                    headers: {
